...
name: huashu-nuwa
description: |
  女娲蒸馏引擎（PsyClaw 随包）：把「学者思维」或「期刊/文体风格」蒸馏成可运行 Skill。
  学者路径：人名/学派 → 心智模型、启发式、表达 DNA、反模式与诚实边界（非数字分身）。
  期刊路径：期刊名或文体（如 Nature 简报、APA Results、Psych Science 审稿口吻）→ 结构模板、语气约束、必检项与禁写项。
  触发：「造skill」「蒸馏XX」「女娲」「学者镜头」「期刊风格」「投稿口吻」「做一个XX视角」。
  English: "distill [scholar|journal]", "nuwa", "journal style skill", "APA results voice".
---
...
女娲不是复制人或伪造期刊，是**提炼可运行的写作/思维操作系统**。

两类随包主路径：

| 类型 | 蒸馏什么 | 产出 Skill 擅长 |
|------|---------|----------------|
| **学者镜头** | HOW they think（心智模型、启发式、表达 DNA、反模式） | 方法学顾问、设计批评、解释框架 |
| **期刊/文体风格** | HOW the venue writes & judges（结构、语气、必检项、禁写项） | 投稿口吻、Results/Methods 段落、审稿回应骨架 |

**关键区分**：捕捉的是框架与约束，不是伪造署名、假引文或「冒充某刊编辑部」。
...
| 用户输入 | 路径 | 示例 |
|---------|------|------|
| 明确的人名/学派 | **学者直接路径** → Phase 0A | 「蒸馏卡尼曼」「做一个 Gelman 镜头」 |
| 期刊名 / 文体 / 投稿口吻 | **期刊风格路径** → Phase 0J | 「Nature 简报风格」「APA Results 口吻」「Psych Science 审稿腔」 |
| 模糊的需求/困惑 | **诊断路径** → Phase 0B | 「我想提升决策质量」「投稿总被说太啰嗦」 |

学者路径产出须遵守 PsyClaw 边界：非本人、非治疗、统计计算仍委托外部库/MCP。  
期刊路径须标注「风格透镜」：不伪造接收函、不保证录用、不编造该刊未公开的内部标准。
...
1. **对象**：具体期刊（如 *Psychological Science*）还是通用文体（APA 7 Results、登记报告、简报）？
2. **用途**：起草 / 改写 / 自检清单 / 模拟审稿意见骨架？
3. **语种**：中文心理学期刊 vs 英文主流刊？
4. **新建 or 更新**：是否已有同名风格 Skill？

调研维度（标准档）：

1. **结构合同**：IMRaD / 登记报告 / 简报字数与小节习惯  
2. **证据与统计报告习惯**：效应量、CI、探索 vs 确证措辞（对齐 PsyClaw gates，不发明统计）  
3. **语气 DNA**：被动/主动、hedging、禁夸大因果  
4. **必检项**：图表题注、伦理声明、数据可用性、利益冲突  
5. **反模式**：灌水引言、p-hacking 话术、未验证引用、「本研究首次」空话  

产出 SKILL.md 须含：使用说明与免责声明、结构模板、语气规则、自检清单、诚实边界（「非该刊官方指南全文」）。

确认后 → 按学者路径同类的调研→提炼→写盘流程推进（Phase 0.5 起），但模板字段改为期刊风格合同而非人物心智模型。

---

### Phase 0A: 需求澄清（学者直接路径）
