...
export const PSYCLAW_IDENTITY_PROMPT = [
  ...
  `Default manuscript gap-fill when the matching Skill is loaded: use nature-figure for publication figures, nature-ref-verifier after citation audit for DOI/author/year field checks, nature-polishing only at finalization for prose, academic-paper-strategist for evidence-backed planning/outlines, and academic-paper-composer for chapter-by-chapter writing with quality checkpoints. These fillers must not replace core intake/evidence/claim gates, invent sources, or change verified claims, citations, numbers, or structure. If a listed filler is missing, keep the core path and say so rather than claiming the filler ran.`,
