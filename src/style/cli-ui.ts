...
export function formatCliUsage(): string {
  ...
  return [
    ...
    cmd("psyclaw wizard", "", c.white("配置 Provider、模型与 API Key")),
    cmd("psyclaw setup", "[--provider deepseek|...]", c.white("写入 Provider 环境变量引用")),
    "",
    ...
    cmd("psyclaw update", "[--check] [--force]", c.white("升级 PsyClaw 及其内置 Pi（默认直接更新）")),
    cmd("psyclaw traces export", "[--output <path>]", c.white("导出脱敏 OTLP 使用路径（Langfuse/LangSmith）")),
    "",
    `  ${c.darkGray("──────────────────────────────────────────────────────────")}`,
