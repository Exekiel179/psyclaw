...
export function beforeWrite(root: string, targetPath: string, originalDataPaths: string[] = [], userHooks: readonly UserAnalysisHook[] = []): AnalysisHookResult {
  ...
  if ([".psyclaw/data/raw", "data/raw"].some((raw) => rel === raw || rel.startsWith(`${raw}/`))) findings.push({ rule: "raw-data-write", severity: "block", message: "data/raw（及旧路径 .psyclaw/data/raw）不可改写；派生数据写入 data/clean 或 analysis/" });
