...
function panelExternalToolInstallTask(root: string, item: Record<string, unknown>): string {
  return [
    ...
    "用户已在 Panel 中明确点击安装。读取官方安装说明，执行安装并验证版本或最小命令；不要把凭据值输出到聊天、日志或命令参数，可用官方检查命令只报状态。",
...
function panelMcpInstallTask(root: string, item: Record<string, unknown>, prep?: Record<string, unknown>): string {
  ...
  return [
    ...
    "安装完成后实际启动一次服务器或执行等价的最小连通性检查，确认启动命令和工作目录有效。不要把 API Key、令牌及其他凭据值输出、复制或提交；连通性检查只汇报通过/失败。",
