...
def _binary_xy():
    # 一个连续预测变量与二元结局；每个 x 水平都有 0/1 重叠（非分离），
    # 使逻辑斜率有限且良好确定——两种实现都能收敛到同一 MLE。
    xs = [-2, -2, -2, -1, -1, -1, 0, 0, 0, 1, 1, 1, 2, 2, 2]
    ys = [0,  0,  1,  0,  1,  0, 0, 1, 1, 1, 0, 1, 1, 0, 1]
    return xs, ys
...
def _monotone_data():
    # x 越大类别越高（单调趋势），但各 x 水平留有重叠（非分离），
    # 使 β 有限、SE 可计算且显著为正。20 例
    xs, ys = [], []
    for x, cat in [(1, 1), (1, 1), (1, 2), (2, 1), (2, 2), (2, 2),
                   (3, 2), (3, 2), (3, 3), (4, 2), (4, 3), (4, 3),
                   (5, 2), (5, 3), (5, 3), (2, 1), (3, 2), (4, 3), (1, 1), (5, 3)]:
        xs.append(float(x))
        ys.append(cat)
    return xs, ys
