...
- ⬜ feat-138 候选:agent 初始化 MCP 冷启动过重——/agent on 时 merge_mcp_tools
  逐个启动所有健康 MCP 子进程 list_tools(browser 29 工具 npx 冷启最慢 + sequential
  + web),首次响应被拖长(实测 deepseek 会话 280s 窗口内未答完)。改惰性:
  目录先登记工具名(从 registry provides 或缓存),真正被调用时才起对应子进程;
  或按意图路由(feat-113)只对本轮相关的 MCP 组做 list_tools。
