...
## 2. 未完成 / 进行中 —— v0.5(2026-07-07 立项:编排纵深——agent 真正会用 MCP)

把「统计外移到 MCP」从目录/健康检查兑现成 agent 循环里**可调用**的工具;顺带升级长会话蒸馏质量。

- 📋 feat-039 MCP stdio 客户端(`psyclaw/mcp/client.py`):JSON-RPC 2.0 over stdio,
  initialize/tools/list/tools/call,超时 fail-safe,`python`→sys.executable 兜底本机。
- 📋 feat-040 agent 循环接入 MCP 工具:build_tools 合并已启用+健康的 MCP 工具(惰性连接、
  `mcp__<server>__<tool>` 前缀、side_effect fail-closed 批准、连接失败不拖垮工具集)。
- 📋 feat-041 compact_history LLM 蒸馏升级:可选用 provider 把丢弃轮次蒸馏成结构化摘要,
  provider 缺失/异常 → fail-safe 回落现有规则蒸馏(zero-dep 契约不破)。
- 📋 feat-042 v0.5.0 发布收尾:版本 0.5.0 + CHANGELOG + docs + 快照。
