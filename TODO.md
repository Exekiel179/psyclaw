...
- ⬜ feat-096 候选:check/jars 判据升级——①效应量缺 CI 现仅 ⚠ 警告,按铁律应
  阻断;②「已报告剔除人数及理由」被事后剔除凑显著的稿件直接拿 ✓,应联动检查
  剔除标准是否预先定义;③横断面/相关设计 + 因果动词(导致/降低/证明)的启发式
  拦截;④多个亚组+无校正+确证表述组合拦截。
- ⬜ feat-097 候选:cite-check 零语料一致性核查——文内引用 ↔ 参考文献列表双向
  比对不需要任何检索语料(实测:正文 Smith & Johnson (2024) 不在文献表、文献表
  Davis (2015) 未被引用,当前全靠「需人工核」放过)。
