...
## 2. 未完成 / 进行中 —— v0.3(2026-07-07 立项:安全加固 + 长会话可靠性)

主题:agent 执行面安全(外部安全审查 2026-07-07 标记 HIGH/MEDIUM)+ 「对话长期维持」的另一半。

- 📋 feat-031 shell 执行 fail-closed(**HIGH**):repl.py run_commands 现仅靠 _DANGEROUS_RE
  正则拒绝清单把关 LLM 生成的 shell(拒绝清单≠安全边界)——改为 shell 类命令**每条**须
  confirm(fail-closed),_DANGEROUS_RE 降级为确认提示里的 ⚠ 标签;psyclaw 进程内命令保持自动。
- 📋 feat-032 save_file 路径允许清单(**MEDIUM**):toolloop._save 现用 confirm=lambda p:True
  写任意路径——改为 resolve 后必须落在 project_dir 内,拒软链接/凭据类路径(.ssh/.aws/.netrc/.env/*.pem)。
- 📋 feat-033 长会话上下文修剪:toolloop convo 无界增长(24 轮 × ≤6000 字符回灌)——循环内
  滚动修剪(保任务头+最近轮,旧工具结果压缩为摘要行);审视 compact_history 并补质量测试。
- 📋 feat-034 normalize_type 中文研究类型别名(『元分析』→meta 等,高频入口小修)。
- 📋 feat-035 v0.3 发布收尾:版本统一 0.3.0(pyproject 现 0.1.0 / __version__ 现 0.2.0 不一致)
  + CHANGELOG v0.3 段。
