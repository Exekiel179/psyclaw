...
| 生态 / 记忆 / 交互 | feat-008 三层记忆 · feat-010 REPL+路径注入+MD 渲染 · feat-011 harness 契约 · feat-017 外部技能包发现(接 AcademicForge/AJS)· feat-018 skill 路由推荐 |
...
| # | 任务 | 说明 | 验收 |
|---|------|------|------|
| 🚧 feat-013 | 会话持久化 session store(SQLite+FTS5) | 在既有 `recall.ContextArchive`(已按 session 分轮存 SQLite)之上补:**sessions 元数据表** + **FTS5 全文检索**(无 FTS5 环境回落 LIKE)+ 会话生命周期命令 **resume / rename / list / search**(REPL slash + 顶层 CLI);替代纯内存 `context.compact_history` 跨会话丢失。 | 新会话可 resume 续接;rename 生效;全文检索命中;FTS5 缺失回落可跑;测试覆盖 store + 回落 |

---

## 3. 候选后续(未排期,来自 progress.md 收尾建议)

- 把 **skill 推荐**(feat-018)挂进 **auto-loop 感知阶段**:发现某 `<type>-loop` 时顺带列相关外部技能包。
- 把 **AcademicForge** 包成一个 **MCP connector**(而非仅发现 SKILL.md)。
- **期刊画像层**扩到更多刊,并加**字数 / 版块**的稿件级核对(现只做引用风格 + 数据可得性)。
...
| 后续 | 编排纵深(auto-loop)· 学术诚信(cite-check/provenance)· 期刊定制 · 生态接入 | ✅ feat-012…019;剩 feat-013 |
