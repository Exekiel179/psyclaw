...
- ⬜ feat-098 候选:稿件数值×措辞矛盾检测(jars 诚信启发式扩展)——
  ① p≥.05(如 p=.051)紧邻「差异显著/得到确证」→ block(正则可判,实测漏网);
  ② 效应量 d<0.2 却称「强效应/意义重大/建议推广」→ warn;
  ③ α<.60 却称「信度良好」→ warn(实测 α=.58 洗白放行)。
- ⬜ feat-099 候选:因果设计检测泛化 + 选择性报告信号——纵向/观察性/追踪/两波
  字样 + 因果动词 + 无随机化 → block(现只认「横断面/相关」关键词,实测两波
  追踪稿绕过);「限于篇幅仅报告(达到)显著」类措辞 → outcome reporting warn。
- ⬜ feat-100 候选:analysis 路径对齐 feat-094——①recommend_analysis 不再拿第一个
  数值列当 DV(实测选了基线 pre_score 当结局变量,静默产出无意义统计;应偏好
  post/outcome/score 语义或经设计步确认);②生成脚本内嵌 to_numeric 清洗与
  剔除呈报;③重复被试 id / 常见缺失码(99/-999)/ 微型组(n<5)三类数据质量
  警示进画像步(实测全部零检测)。
