...
- ✅ feat-161 scale 重定位:careless.py 虚伪作答体检(longstring/漏答/直入式,零统计,
  跑原始应答不被反向计分掩盖)接入 score + reliability_script 委托 pingouin 的 α 脚本
  (score --reliability);α/ω/CFA 外移(psyclaw/psych/careless.py·reliability_script.py·scales.py)。
- ✅ feat-162 cite 重定位:删方法学背书静态库 evidence.json(不现实),cite 改引文核查
  (反杜撰,复用 citations.run_citation_audit);背书注入/print_evidence 移除
  (psyclaw/psych/evidence.json 删·knowledge.py·context.py·cli.py·repl.py)。
- ✅ feat-163 method 重定位:method 从方法词典改成方法学 skill 路由——sample-size
  (功效分析,power_script 委托 statsmodels 解 N)+ confound-control(无关变量控制,纯设计);
  method_skills 意图别名匹配,未命中退回词条(psyclaw/skills/{sample-size,confound-control}/
  SKILL.md·psyclaw/psych/{power_script,method_skills}.py·cli.py)。
- ✅ feat-164 cite 引用文章:元数据→APA7 参考文献 + 文内引用(1/2/3+ et al./en dash/>20 省略),
  纯字符串无统计;cite 双模 --make <json> 生成 / 否则核查(psyclaw/psych/reference.py·cli.py)。
  **三部曲全部完成。**
...
**真实会话二次复盘(2026-07-17,chat_20260717-003855)**:
- ✅ feat-151 跑脚本环境修复:_run_env 注入 PYTHONPATH(生成脚本可 import psyclaw,
  治 ModuleNotFoundError)+ _normalize_interpreter(python→python3)+ 提示示例改
  python3(psyclaw/repl.py)。
- ✅ feat-154 解释器归一跨平台化:不硬编码 python3(对 win 反向坑),改按平台
  优先级取本机实际可用解释器(win python→py→python3,posix python3→python),
  双向自适配(psyclaw/repl.py)。
- ✅ feat-152 概念图别手画:capability_map 引导 + detect_reinvention 检测手画概念
  框架图(matplotlib+关坐标轴+箭头)→ graphviz/mermaid 纠偏(psyclaw/context.py)。
- ✅ feat-153 命令联想诚实化:input_backend/input_hint 随实际 backend 说实话
  (readline 只 Tab 补全不假装 ↑↓ 下拉)+ 引导装 prompt_toolkit(psyclaw/ui_input.py·repl.py)。
