...
当前**无未完成 feature**——feat-001…062 全部 `done`。**v0.11.0 已发布(2026-07-09)**,
主题=REPL 交互体验大修 + 错误自学习闭环 + 图片内联渲染(feat-055~061),详见 `CHANGELOG.md`。
...
- ✅ feat-074 provenance 深化:`build_replication_declaration` 生成 replication-package
  声明(脚本 sha256+数据指纹+环境,可直接放进稿件);新门禁 `REPRO.replication_package`
