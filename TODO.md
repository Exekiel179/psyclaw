...
- ⬜ feat-126 文件面 + 私密/编码表:private_paths 数据禁止完整外传,跨信任边界
  强制要求 notes/codebook.yaml 脱敏(真实值→代号),本地处理不拦;覆盖先备份。
- ⬜ feat-127 代码执行面:AST/模式静态筛恶意(硬拒)+ 科研意图白名单放行不打断
  + 资源上限(超时/内存/子进程);灰区才问——不频繁中断正常任务。
- ⬜ feat-128 网络面:学术 API/机构库/MCP 白名单放行,上传默认拒,私密数据不出网。
- ⬜ feat-129 start 澄清式沙箱配置:意图澄清 → 选 skill → 询问是否启用沙箱 →
  按启用 skill 的能力清单取并集配置策略(最小权限),落 sandbox.yaml。
...
- ⬜ feat-117 发散检索(创新任务):recall/语义检索 mode=diverge,高相关命中
  之外额外采样中等相关(0.3~0.6 带随机)卡,给设计/头脑风暴造远距联想材料;
  常规任务不启用。
