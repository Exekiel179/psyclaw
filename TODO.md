...
## 2. 未完成 / 进行中 —— v0.6(2026-07-07 立项:多轮对话 + 工具调用稳)

用户重点:多轮对话稳、工具调用不出问题。审计工具循环发现真实脆弱点(已实测复现):

- 📋 feat-043 参数规范化校验:parse_tool_calls 对 args 为 list/双重编码 JSON 字符串时,
  内置工具 `a.get` 崩且 `_exec_tool` 误标 ok=True。修:规范化 args(JSON 字符串→解析、
  非 dict→报错引导)、name 须非空字符串、工具异常改标 ok=False。
- 📋 feat-044 无进展检测:重复相同 (name,args) 调用 / 空回复不再空转到 max_iters,
  有限纠偏后 `stopped=no_progress` 收敛(不静默、不卡死)。
- 📋 feat-045 消息序列不变量:绝不向 convo 追加空 content 消息、final 空则重试一次、
  保持角色交替——防 provider 400。
- 📋 feat-046 v0.6 发布 + 多轮工具调用集成测试(证明不出问题)。
