...
- ⬜ feat-105 候选:chat 未运行先造「输出示例」——第三轮 3d 次实测:save 脚本后
  模型附了一段带具体统计量的「输出示例(基于你提供的已清洗数据)」,t=4.202/
  d=2.101 全是编的且与真值不符(真跑出来 t=4.741/d=2.35)。lean_core 应加:
  脚本未运行前绝不给带具体数值的示例输出,要么运行要么明说"运行后才有数值"。
- ⬜ feat-106 候选(低优先):路径注入误报——用户消息含 outputs/compare_basic.py
  时打出「[文件未找到: /compare_basic.py]」,路径识别把子路径截半;不影响功能,
  但提示噪音(psyclaw/path_ingest.py)。
- ⬜ feat-104 候选:lit matrix 文献矩阵——从 notes/lit_search.json 或导入的
  桥接结果表生成矩阵骨架(标题|作者年份|研究对象|方法|形式/场景|发现|局限|纳入,
  「待核查/全文未获取」约定),条目注册进 evidence_map(cite-check 语料打通);
  lit import <bridge_results.md> 导入外部检索结果。
