...
- ⬜ feat-139 进行中:start 按目标期刊装载 AJS 技能包(设计已定稿:
  docs/superpowers/specs/2026-07-15-ajs-journal-pack-install-design.md)——
  psyclaw/ajs.py(list_packs/resolve_pack/install_pack 稀疏检出)+ mirror github
  回退 + loader 三层 glob + cli start 目标期刊步 & journal install 子命令。
