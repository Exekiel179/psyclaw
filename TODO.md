...
- ⬜ feat-145 命令实时输出:_run_shell_cmd 的 capture_output=True 全吞输出,长脚本
  空屏焦虑——改流式边跑边显示,完整输出仍回传模型。
- ⬜ feat-146 斜杠命令回显 + 输入 / 联想(部分分支静默,用户分不清是否生效)。
- ⬜ feat-147 输出框配色美化。
- ⬜ feat-148 提示强化:「我来跑」不是「请你跑」+ 禁止一次挤多个问题进选项。
- ⬜ feat-149 空回复诊断(deepseek provider 空回复自动恢复根因)。
