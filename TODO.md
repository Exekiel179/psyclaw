...
## 2. 未完成 / 进行中 —— v0.4(2026-07-07 立项:provider 健壮性 + agent 可观测)

继续深化「对话长期维持」:网络瞬断不该杀死一次 agent 长任务;跑过的 agent 任务要可回看。

- 📋 feat-036 provider 网络层健壮性:_post_sse 现在 HTTP 错误只抛裸异常(REPL 只见
  「provider 错误」),无重试——429/5xx/网络异常指数退避重试(≤3 次,仅首字节前),
  HTTP 错误读响应 body 显性化报错(stdlib only)。
- 📋 feat-037 agent 运行痕迹持久化:toolloop 跑完的 trace/final 现在只活在当轮终端——
  落 `.psyclaw/agent_runs.jsonl`(时间/任务/轮数/停止原因/工具调用摘要),
  `psyclaw agent --history [n]` 回看最近 n 次。
- 📋 feat-038 v0.4 收尾:README/docs/COMMANDS 同步 v0.3+v0.4 变化(安全确认行为是
  用户可感知的 breaking 体验,必须写清)+ progress/session-handoff 快照。
