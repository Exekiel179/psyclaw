...
- ⬜ feat-115 统一衰减生命周期:activation = strength·exp(-λ·days),命中刷新,
  低于阈值休眠(不注入可复活),超长期零复用删除(先归档);情景档案不删原文
  只做检索时近加权;升级过人审、降级全自动(错学比忘记贵)。
- ⬜ feat-116 睡眠整合(psyclaw sleep):重放(情景→蒸馏候选语义/程序卡,
  含成功路径 playbook,补"只从失败学"缺口)→ 合并(N 条具体→1 条原则,
  feat-112 手工合并的自动化)→ 衰减结算 → 联想铺路(kg 建边);
  产物一律 pending 待确认,无 LLM 规则兜底。
