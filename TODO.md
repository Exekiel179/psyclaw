...
- ⬜ feat-141 协助水平:psyclaw assist novice|standard|expert(全局 config
  assist_level),lean_core 按水平注入术语解释与生成代码注释密度指令。
- ⬜ feat-142 annotate 命令:给代码写注释(按 assist_level 定密度,默认 stdout,
  --write 落盘带 .bak)+ --review 审查意见;LLM 链路,mock provider 明确降级。
