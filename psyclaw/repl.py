...
_READ_OPEN_SYSTEM = (
    "\n# 读取本地文件(当前:开放模式)\n需要看本地文件内容时,第一人称「我来读」并"
    "**当场直接输出 read 块**,不要说你无法读取,**绝不要让用户手动打开文件、"
    "用编辑器/Excel 查看、粘贴内容或替你报数字**——那是你自己能做的事:\n"
    "```read\n<文件路径>\n```\nPsyClaw 会自动读取并把内容(数据 CSV 给结构+少量样例行;"
    "PDF 抽正文;受保护的 data/raw 会被拒绝)回传给你,你据此继续,不必等用户。")
_READ_SAFE_SYSTEM = (
