---
name: ars
description: Academic Research Skill — 端到端心理学研究总编排：文献调研→实验设计→统计分析→论文写作，全程受 PSYCLAW 质量规范约束。当用户要做心理学研究、跑统计、写论文、做文献综述、设计实验时触发。
category: domain
status: legacy
metadata:
...
- **澄清先行(硬规则)**:正式研究开始前,研究准备清单(17 个研究准备项)必须全部 resolved。
  用户提出研究想法时,第一动作是 grill-me 式逐项澄清(一次一题,带推荐答案),
  前置检查 CLARIFY.complete 未通过时暂停 /research
- **决策必有背书**:每个设计/分析决策引用方法学文献(背书库 `psyclaw cite`),
  质量检查 DESIGN.evidence 校验决策台账
- **输出走 APA7 引擎**:论文产出经 `psyclaw export` 确定性模板(docx+md),不手排版面
- 通用任务在 Chat 中运行;明确研究流程使用 `psyclaw run <类型>`
