...
# 图件完整性(feat-194)——用户实测:一篇 NHB 稿的主路径图从没生成,交付稿里
# 赫然留着 ![Path diagram — to be generated](…);次要 moderation 图生成了却没内嵌、
# 且存到 figures/ 而链接指向 outputs/figures/ 成孤儿。质检此前完全不看图,静默放行。
# 图是论文内容的一部分,不是可选项:占位=缺内容,断链=图没生成/路径错,均须拦截。
_IMG_RE = re.compile(r"!\[([^\]]*)\]\(([^)]*)\)")
_FIG_PLACEHOLDER_PAT = [
    r"to be generated", r"to be (?:inserted|added|created|drawn|made)",
    r"will be (?:generated|inserted|added|created)",
    r"\bplaceholder\b", r"\bTBD\b",
    r"待(?:生成|插入|补充|添加|绘制|制作)", r"占位", r"尚未生成", r"暂未生成",
    r"此处(?:应有|插入|放置?)",
]


def _body_only_for_scan(text: str) -> str:
    """截去参考文献区,只留正文供诚信启发式扫描。

    复用 citations 的标题行判据(单一真源:`## 参考文献` / `References` /
    `证据图谱` 等);该模块不可用时退回原文,宁可误报也不静默漏检。
    """
    try:
        from psyclaw.psych.citations import _body_only
        return _body_only(text)
...
def integrity_flags(text: str) -> list[dict]:
    ...
    **只扫正文,不扫参考文献表**(feat-193)。用户实测:参考文献里
    "Causes of employee turnover" 这类**真实文献的标题**被判成「因果表述越界」,
    而模型为了过检竟打算换掉那篇贴切的真实文献——检查器倒逼学术失真,
    比不检查更糟。作者对文献标题没有改写权,标题里的词不构成作者的因果主张。
    """
    text = _body_only_for_scan(text)
    flags: list[dict] = []
    ...
    # 图件占位未兑现(feat-194):![alt](path) 的 alt 自认「待生成/占位」——
    # 缺图=缺内容,不得交付。仅扫 alt 文本(高精度),避免误伤正文里谈论占位符的行文。
    for m in _IMG_RE.finditer(text):
        ph = _hit(m.group(1), _FIG_PLACEHOLDER_PAT)
        if ph:
            flags.append({
                "id": "I.figure_placeholder", "severity": "block",
                "label": "图件占位未兑现:交付稿仍带「待生成 / to be generated」占位图",
                "evidence": m.group(1).strip()[:60],
                "fix": "生成真实图件替换占位——主图(路径图/森林图/交互作用图)是论文"
                       "内容的一部分,不是可选项;补齐后再交付,不留 to-be-generated 空洞。"})
            break
    return flags


def figure_file_flags(text: str, base_dir: str | Path) -> list[dict]:
    """图片链接指向的文件是否真实存在(需草稿所在目录)——feat-194。

    交付稿的每个本地 ``![](…)`` 都应能解析到真实文件。链接指向不存在的路径 =
    图没生成或相对路径写错(实测:moderation 图存到 figures/ 而稿件写 outputs/figures/
    成孤儿;主路径图链接的 png 干脆没生成)。远程 / data-uri 链接跳过(交由联网核验)。
    只扫正文(复用参考文献截断),与 integrity_flags 同口径。
    """
    flags: list[dict] = []
    base = Path(base_dir)
    seen: set[str] = set()
    for m in _IMG_RE.finditer(_body_only_for_scan(text)):
        raw = m.group(2).strip()
        if not raw or raw.startswith(("http://", "https://", "data:", "#")):
            continue
        path = raw.split()[0].strip("<>")   # 去掉 ![](a.png "title") 的 title
        if not path or path in seen:
            continue
        seen.add(path)
        if not (base / path).exists():
            flags.append({
                "id": "I.figure_missing_file", "severity": "block",
                "label": "图片链接指向的文件不存在(图未生成或相对路径写错)",
                "evidence": path[:80],
                "fix": "生成该图并放到链接位置,或修正相对路径;交付稿的每个 "
                       "![](…) 都必须解析到真实文件,否则读者看到的是断图。"})
    return flags
...
def check_draft(text: str, research_type: str = "quant",
                base_dir: str | Path | None = None) -> dict:
    """对论文文本跑 JARS 检查，返回结构化结果。

    base_dir 给定时(草稿所在目录),额外校验图片链接指向的文件是否存在
    (feat-194 图件完整性);不给则跳过文件级校验,行为与旧签名一致。
    ...
    if base_dir is not None:                          # feat-194:图件文件级完整性
        integrity = integrity + figure_file_flags(text, base_dir)
...
def jars_cli(argv: list[str]) -> int:
    ...
    fig_base = src.parent if args.draft else None    # feat-194:文件输入才能核图路径
    result = check_draft(text, args.research_type, base_dir=fig_base)
