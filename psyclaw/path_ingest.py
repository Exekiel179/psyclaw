...
def _build_path_re() -> re.Pattern[str]:
    ...
    # feat-106:`/` 前是 ASCII 字母数字/点/波浪线时不是路径起点——否则
    # 「保存为 outputs/compare_basic.py」会被从中间切出 /compare_basic.py
    # 误报「文件未找到」(第三轮评估实测);中文前缀(如「地址是/tmp/x」)不受影响。
    unix_abs = r'(?<![A-Za-z0-9_.~/-])(/(?:[^\s,;()\[\]"\'<>]{1,249}))'
    tilde = r'(?<![A-Za-z0-9_.~/-])(~[^\s,;()\[\]"\'<>@\n]{1,250})'
    rel = r'(?<![A-Za-z0-9_~/-])(\.\.?/[^\s,;()\[\]"\'<>]{1,250})'
    return re.compile(
...
def _data_metadata(path: Path) -> str:
    ...
    lines = [
        ...
        # 会诱导模型调用不存在的命令)。当前正确路径 = run analysis 生成可复现脚本。
        f'[📊 全量分析: psyclaw run analysis "{path}"(生成可复现脚本,'
