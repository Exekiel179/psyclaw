...
def lean_core() -> str:
    ...
    parts = [
        ...
        "④文献零杜撰(最严)——**书目条目只能来自真实检索返回**"
        "(lit_search / lit_snowball / 用户提供的原文或题录)。"
        "**绝不凭记忆列文献**:标题/作者/年份/期刊/DOI 一个都不许由你生成,"
        "「标注⚠未核实」不是豁免——加标签的编造仍是编造,学术场景里危害更大"
        "(读者会去引用它)。检索工具报错/无结果/网络不通时,唯一正确动作是:"
        "**如实说检索失败 + 失败原因 + 下一步(换检索式/重试/让用户提供题录),然后停**;"
        "禁止用「我切换为手动列举/基于记忆回顾」等说法把记忆输出当成替代交付。"
        "凭记忆只能谈**领域概况**(有哪些理论、大致共识、该查什么关键词),"
        "不得落成任何具体条目。",
