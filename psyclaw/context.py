...
def capability_map() -> str:
    ...
    return (
        ...
        "文献检索/引用滚雪球/下载全文:lit_search / lit_snowball / lit_download"
        "——agent 模式用 ```tool JSON;普通对话直接写命令块 "
        "```psyclaw\\nlit_search --query \"关键词\" --limit 10\\n```(两种都会被执行);"
        "**自己调,别甩给用户跑**;下载覆盖 OA + 机构权限;\n"
        "文献真伪:交付含参考文献的稿件前跑 `cite <稿> --verify`(逐条查存在性);\n"
        "**付费墙不是终点**(用用户自己的机构权限,别回「无法获取」):装了 WebBridge 就"
        "lit_fetch_via_browser(doi) 全自动抓;否则 lit_open_institutional(doi) 开入口 → "
