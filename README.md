...
当前版本：`0.27.21`。正式命令、用户配置目录和后续发布统一使用 `psyclaw`。
...
```powershell
npm install -g psyclaw@0.27.21
```

如果本机 npm 配置把 registry 误写成带有 `~/` 的地址，请显式指定官方源：

```bash
npm install -g psyclaw@0.27.21 --registry=https://registry.npmjs.org/
```

中国大陆网络较慢或无法访问官方源时：

```powershell
npm install -g psyclaw@0.27.21 --registry=https://registry.npmmirror.com
```

PsyClaw 启动时会确保 `ripgrep` 和 `fd` 可用。检测到 `HTTP_PROXY`、`HTTPS_PROXY` 或 `ALL_PROXY` 时，通过代理访问 GitHub 官方 API 与 Release；检测到国内 npm registry 时，版本查询和压缩包统一通过 `https://gh-proxy.com/`。可用 `PSYCLAW_GITHUB_MIRROR=https://your-mirror.example/` 指定自建 HTTPS 镜像。
