...
当前版本：`0.29.8`。正式命令、用户配置目录和后续发布统一使用 `psyclaw`。
...
```powershell
npm install -g psyclaw@0.29.8
```

如果本机 npm 配置把 registry 误写成带有 `~/` 的地址，请显式指定官方源：

```bash
npm install -g psyclaw@0.29.8 --registry=https://registry.npmjs.org/
```

中国大陆网络较慢或无法访问官方源时：

```powershell
npm install -g psyclaw@0.29.8 --registry=https://registry.npmmirror.com
```
