...
当前版本：`0.27.3`。本仓库以 `psypi v0.4.1` 的完整工作台为功能基线完成改名；`psypi` 仓库保留为历史仓库，正式命令和后续发布均使用 `psyclaw`。
...
```powershell
npm install -g psyclaw@0.27.3
```

中国大陆网络较慢或无法访问官方源时：

```powershell
npm install -g psyclaw@0.27.3 --registry=https://registry.npmmirror.com
```
