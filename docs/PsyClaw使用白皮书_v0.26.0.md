...
系统会启动仅绑定 `127.0.0.1` 的本地工作台并在浏览器中打开。当前面板聚焦三类视图：项目文件只读阅读、Skill/MCP 生态管理、Provider 与成本配置。在 Provider 页可选择模型和保存 API Key，也可选填 Base URL 以指向 OpenAI 兼容网关；留空则继续使用该模型的预设地址。研究文档和运行事实通过文件与快照展示，不在面板中编辑或提交人工裁决。

面板不启动独立聊天助手，也不把项目内容发送到新的 RPC 会话；文件阅读和运行状态均来自本地只读投影。
