...
- `cli.llm_call` — interactive Pi `turn_start`/`turn_end`（`phase=pi_turn`）、`PiModelGateway.complete`、只读 RPC worker，以及 ARS 审稿席位 / 编辑合成 / re-review 各阶段 RPC（`phase=ars_review_seat_*` / `ars_synthesis` / `ars_rereview_*`）
...
挂载点：interactive Pi `turn_end`（每轮 assistant 消息；主路径）、`agent_end`（turn 钩子未发出时的兜底，避免与 `turn_end` 重复计数）、RPC worker 返回的 session 事件（`message_end` / `agent_end`）、`PiModelGateway.complete`、ARS multi-agent 审稿席位 / 编辑合成 / re-review 各阶段 `promptAndWait`。Interactive 会话走 Pi 的 streaming 循环，**不会**经过 `PiModelGateway.complete`。
...
Skill / 推荐状态写入失败时，CLI / 面板会尽量给出带路径的中文错误，并把 errno、脱敏路径、scope 送进 Sentry / `agent_error`（见 `filesystemErrorContext`）。
