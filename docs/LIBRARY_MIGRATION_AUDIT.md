...
> ⚠️ **已被更彻底的方案取代(2026-06-26)**:不再逐模块迁移到库,而是把统计层
> **整体删除**——所有手写统计模块(连同 CLI 命令与测试)已 git rm,统计计算外移到
> 外部成熟库/MCP。本文件保留作历史审计记录;`multiple_testing` 是删除前唯一完成的
> 库迁移样例。当前命令清单见 `docs/COMMANDS.md`,重定位说明见 `CLAUDE.md` / `progress.md`。
