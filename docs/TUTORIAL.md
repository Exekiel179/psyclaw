...
一个**心理学研究编排** CLI:把 澄清 → 文献 → 设计 → 写作 → 评审 → 学术门禁 串起来,
...
```bash
python -m psyclaw doctor        # 环境自检:看配置/MCP/门禁状态
...
| 模型自己要读文件 | **默认开放模式**:它会自动读并继续分析;`/safemode` 切安全模式=一切读取须你 `@` 显式引用 |
| 保存它写的内容 | 直接说"存到 method.txt"——自动落盘(覆盖前会确认;绝不写 `data/raw`) |
...
正式开工前 PsyClaw 要求完成 17 槽位澄清(防 HARKing)。但**你有权显式跳过**:

```bash
python -m psyclaw lit-loop "正念训练对焦虑的干预效果" --skip-gates
```

- 流程照跑:检索(OpenAlex/EuropePMC 合法 OA)→ PRISMA 筛选 → 合成综述 → 审稿模拟;
- 跳过的门禁**留痕** `notes/gate_skips.md`,产物标**探索性**——诚信靠标注,不靠拦。
- 产物:`notes/lit_review.md`(综述)、`notes/evidence_map.json`(构念×文献证据图谱)。

### 路线 B:正式开工(推荐的完整路径)

```bash
python -m psyclaw clarify       # grill-me 式澄清:17 槽位逐项问(有 LLM 时会追问模糊回答)
python -m psyclaw auto-loop     # 自主科研回路:自动发现该做什么 → 派发 → 独立验收 → 记状态
```

`auto-loop` 每轮做五件事:**感知**(从仓库状态推导待办:有目标→综述;有数据表→实证分析;
有效应量表→元分析;有转录稿→质性)→ **派发** → **独立验收**(只认落盘产物)→ **记状态** → **决定**。
每个待办下还会列出你装的相关技能包。派发前会问你确认(全自动加 `--auto`)。
...
```bash
python -m psyclaw analysis-loop data/clean/scores.csv
```
...
python -m psyclaw agent "把综述里的构念整理成清单存 notes/constructs.md" --auto
                                               # 模型自主多步调工具(读/搜/存/查图);副作用默认要批准
```

- **REPL `/agent`**:对话中开 agent 模式,模型自己决定调 search/read_file/save_file/kg_query/recall。
...
| 被"澄清未完成"拦住 | 正式路线先 `psyclaw clarify`;只想探索加 `--skip-gates`(留痕+标探索性) |
| PDF 读出乱码/读不到 | 扫描件/加密件抽不了正文;`pip install pypdf` 提升抽取质量,或先 OCR |
| 模型说"无法读取文件" | 你在安全模式(提示符有 `safe`)。`/safemode off` 放开,或用 `@路径` 引用 |
| 统计命令去哪了 | 外移了:分析流程生成脚本 → `[stats]` 环境或 MCP 跑(见 COMMANDS.md「统计去哪了」) |
| 想让 auto-loop 换个任务 | 它验收不过会自动跳过换下一个;卡在等决策时看 `psyclaw status` |
...
psyclaw status        ←—— 迷路先敲这个
   │
   ├─ 探索:lit-loop "主题" --skip-gates
   ├─ 正式:clarify → auto-loop(自动发现→派发→验收→记状态)
   │        └─ 数据:data/clean/*.csv → analysis-loop → outputs/analysis.py 去 [stats] 跑
