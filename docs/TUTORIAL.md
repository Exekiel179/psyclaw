> **历史文档。** 本页仅保留用于复现 Python 时代的 PsyClaw v0.23.0 教程，不能用于当前 npm 发行版。请阅读 [v0.26.0 使用白皮书](PsyClaw使用白皮书_v0.26.0.md) 和根目录 README。
...
| 它给了 shell/Python 命令块 | 普通命令默认自动执行并实时回传；长任务每 5 秒显示后台状态；危险命令（如强制推送、硬重置、删库）始终确认；`/approval ask` 可改为逐条确认 |
