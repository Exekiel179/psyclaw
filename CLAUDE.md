...
架构细节见 `docs/ARCHITECTURE.md`(历史设计稿 `dev/docs/DESIGN.md`)；开发脚手架(计划/状态/过夜循环)全部在 `dev/` 下：
计划真源 `dev/TODO.md`，过夜任务说明 `dev/PROMPT.md`。
...
1. 读 `dev/TODO.md` + `git log --oneline -15`，选一个未完成、依赖就绪的最高优先级任务。
...
6. 在 `dev/TODO.md` 把该任务标 ✅ 并注明实现文件。
7. `git add -A && git commit -m "<描述性消息：做了什么 + 在哪个文件>"`。

## Harness 契约（machine-checkable，learn-harness-engineering 5 子系统）

状态真源 = `dev/feature_list.json`（features[].status/dependencies/evidence）；计划真源 = `dev/TODO.md`；
人读快照 = `dev/progress.md`；交接 = `dev/session-handoff.md`。结构分见
`node dev/agents/skills/harness-creator/scripts/validate-harness.mjs --target .`。

- **Startup Workflow（Before writing code）**：先 `PSYCLAW_PYTHON=C:/Python314/python ./dev/init.sh`
  跑 Verification Commands（compile + pytest + gates），再读 `dev/feature_list.json` + `dev/progress.md` 接续上下文。
...
- **End of Session（Before ending）**：更新 `dev/progress.md`（Current State/What/Next）与
  `dev/session-handoff.md`（Blockers/Files/Next Session/Recommended Next Step），留下 clean、restartable 的状态。
