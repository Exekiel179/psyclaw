...
PsyClaw 是心理学研究全流程 Agent CLI，纯 Python、**stdlib 零运行时依赖优先**。
入口 `python -m psyclaw`，包根 `psyclaw/`，测试 `python -m pytest -q`。
...
- **零依赖优先**：核心路径不新增强依赖；用重库（pingouin/R/matplotlib）必须保留 stdlib 降级分支
  （参考 `psych/analyze.py` 在 pingouin 缺失时回落 `stats_core`）。
...
3. 实现 + 完善错误处理。
4. 加/改 `tests/test_*.py`，跑 `python -m pytest -q` 全绿；统计数值对照 scipy/pingouin 校验。
