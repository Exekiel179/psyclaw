...
PsyClaw 是心理学研究全流程 Agent CLI，纯 Python、**成熟统计库优先**
（scipy/pingouin/statsmodels/lifelines/factor_analyzer/semopy 为硬依赖，配置时安装）。
入口 `python -m psyclaw`，包根 `psyclaw/`，测试需在装好统计栈的解释器下跑
（本机 `C:\Python314\python -m pytest -q`；msys 默认 python 无统计栈）。
...
- 同一错误尝试 **3 次**仍无进展 → 记录到 `notes/blocked.md`，**切换到下一个任务**，
  不要在一个坑里无限循环。
- 需要不可逆操作（删数据、改测量口径、动门禁判据）→ 不自作主张，写
...
- **成熟库优先，不重复造轮子**：统计计算一律复用成熟库——scipy/pingouin 为主干，statsmodels（回归族/GLM）、
  lifelines（生存）、factor_analyzer（EFA）、semopy（CFA/SEM）按需。它们是**硬依赖**（配置时即安装），
  禁止手写已被这些库覆盖的统计算法（分布函数/参数估计/检验）；模块只保留 CLI/IO/APA-7 格式化/报告等胶水层。
- **不破坏既有契约**：`cli.py` 现有命令行为不回归；门禁只增不偷偷删。
- **学术诚信不可妥协**：效应量+CI 必报、相关≠因果、区分探索/确证、不 p-hacking——
  任何统计/写作产出都要符合 `gates/PSYCLAW.md`。
...
3. 实现 + 完善错误处理。
4. 加/改 `tests/test_*.py`，在装好统计栈的解释器下跑全绿（本机 `C:\Python314\python -m pytest -q`）；统计数值对照 scipy/pingouin 校验。
...
- **Definition of Done**：done only when —— 全量测试绿 + 数值对照 scipy/pingouin + 把
  `command and output` 作为 Verification Evidence 记入该 feature 的 `evidence` 字段。门禁只增不删。
